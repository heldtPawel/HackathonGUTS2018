...
def scan():
	...
	current_turret_heading = turret_heading
	#print(str(idTank) + " | "+str(x) + " | " + str(y))
	#print("Start Head: "+str(current_turret_heading))
	for i in range(18):
		message_in_function = messageServer
...
def readServer():
	while True:
		...
		if GameServer.readMessage()[1] == 18:
			...
			print("-----------------------")
			print(messageServer)
			if messageServer['Id'] == idTank:
				print(messageServer['Id'])
				x = messageServer['X']
				y = messageServer['Y']
				heading = messageServer['Heading']
				turret_heading = messageServer['TurretHeading']
				print(turret_heading)
				health = messageServer['Health']
				ammo = messageServer['Ammo']
				#print(ammo)
...
def main():
	...
	while True:
		...
		#GameServer.sendMessage(ServerMessageTypes.MOVEFORWARDDISTANCE, {'Amount': 15})
		if (iMain % 15)==0:
			print("Turret Heading : " + str(turret_heading) )
			scan_out = scan()
			print(scan_out)
		iMain+=1
		continue
